...
export async function uploadChannelAvatar(
  ...
  // 3. 上传文件到Vercel Blob
  const filename = `channel-avatars/${session.user.id}/${channelId}/${uuidv4()}-${file.name}`;
  const blob = await put(filename, file, {
    access: 'public',
    addRandomSuffix: false,
  });
 ...
 * @returns 是否成功删除
 */
export async function deleteChannelAvatar(channelId: string): Promise<boolean> {
  // 1. 验证用户身份
  const session = await getSession();
  if (!session || !session?.user?.id) {
    throw new Error('请先登录');
