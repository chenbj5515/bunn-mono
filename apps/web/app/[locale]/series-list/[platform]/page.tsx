...
const SeriesListPage: FC<SeriesListPageProps> = ({ params }) => {
  ...
  const cardPositions = [
    ...
    { left: '10%', top: '125%', rotate: '12deg', zIndex: 5 }, // 11
              ...
              >
                ...
                {hasImage && (
                  <div className="relative w-full h-full">
                    ...
                    {/* 悬停时显示的标题 */}
                    {/* <motion.div 
                      className="bottom-0 absolute bg-black/70 opacity-0 p-2 w-full"
                      initial={{ opacity: 0, y: 10 }}
                      whileHover={{ opacity: 1, y: 0 }}
                      transition={{ duration: 0.2 }}
                    >
                      <p className="font-medium text-white text-sm">{poster.title}</p>
                      {poster.description && (
                        <p className="text-gray-300 text-xs">{poster.description}</p>
                      )}
                    </motion.div> */}
                  </div>
                )}
