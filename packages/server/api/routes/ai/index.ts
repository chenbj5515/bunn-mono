...
import { trackTokenCount, checkTokenLimit, getUserTokenLimit } from '@server/redis/token-tracker';
...
const app = new Hono()
  .basePath('/ai')
  .post("/extract-subtitles", async (c) => {
    try {
      // 获取用户会话
...
// 使用更简单的路由结构
// app.route('/extract-subtitles', extractSubtitlesRoute)
